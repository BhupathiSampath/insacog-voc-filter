<template>
 ...
 <div class="max-w-full mx-2 py-1 sm:mx-auto sm:px-6 lg:px-10">
  ...
  <div class="max-w-full mx-2 py-1 sm:mx-auto sm:px-6 lg:px-10">
      ...
      <input v-model="date" class="shadow appearance-none border rounded py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" name="date" type="text" placeholder="Date" v-on:keyup.enter="fetchSomething()">
    ...
    <div class="flex justify-center py-1">
      <button class="flex bg-blue-500 hover:bg-blue-700 text-white font-bold my-1 py-0 px-4 rounded-full" id="show" v-on:click="handlechange">Advance filter</button>
      <button class="flex bg-gray-500 hover:bg-blue-700 text-white font-bold my-1 py-0 px-4 rounded-full" id="show" v-on:click="downloadFile">Download</button>
    </div>
    <div id="book" hidden>
...
export default {
  ...
  async asyncData() {
    ...
    // const count = await axios.get(`${process.env.baseUrl}/count/`)
  ...
  methods: {
    ...
    async fetchSomething() {
      ...
      const ip = await axios.get(`${process.env.baseUrl}/data/?page=${this.page}&days=${this.days}&strain=${this.strain}&ordering=${this.ordering}&state=${this.state}&lineage=${this.lineage}&mutation_deletion=${this.mutation_deletion}&date=${this.date}&gene=${this.gene}&reference_id=${this.reference_id}&amino_acid_position=${this.amino_acid_position}&mutation=${this.mutation}`)
      // const count = ip.data.count
      console.log(ip.data)
      this.ip = ip
    },
    async downloadFile() {
      const csv = await axios.get(`${process.env.baseUrl}/exportcsv/?days=${this.days}&strain=${this.strain}&ordering=${this.ordering}&state=${this.state}&lineage=${this.lineage}&mutation_deletion=${this.mutation_deletion}&date=${this.date}&gene=${this.gene}&reference_id=${this.reference_id}&amino_acid_position=${this.amino_acid_position}&mutation=${this.mutation}`)
