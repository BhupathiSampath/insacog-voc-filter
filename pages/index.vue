  ...
  <div>
    ...
    <div class="inset-10 bg-white w-full pl-4 pb-2 hidden" id="lineage-model">
      <div
        ...
        v-for="event in lineagestraindistribution.data.results"
        :key="event.id"
        :event="event"
      >
        <span
    ...
    <div
      class="fixed hidden w-80 mx-auto mt-5 p-7 grid justify-end"
      id="loader"
    >
      <div class="grid justify-items-center">
        <div class="fixed w-80 mx-auto mt-5 p-7">
          Loading....
          <cube-spin></cube-spin>
        </div>
      </div>
    ...
    <div
      id="lineage-model1"
      class="hidden box-border h-100 w-90 p-4 border-4 mt-2"
    >
      <div class="box-content">
        <section>
          <div class="container mx-auto">
            <BarChart
              :key="random"
              :data="barChartDataWeek"
              :options="barChartOptionsWeek"
              :height="500"
              :width="2000"
            ...
            >
              <table class="min-w-full divide-y divide-gray-200">
                  ...
                  <tr>
                    ...
                    >
                      ...
                      <a
                        ...
                        v-on:click="fetchSomething((ordering = 'date'))"
                      >
                        &#x2193;
                      </a>
                      <a
                    ...
                    >
                      ...
                      <a
                        ...
                        v-on:click="fetchSomething((ordering = 'strain'))"
                      >
                        &#x2193;
                      </a>
                      <a
                    ...
                    >
                      ...
                      <a
                        class="cursor-pointer"
                        v-on:click="fetchSomething((ordering = 'state'))"
                      >
                        &#x2193;
                      </a>
                      <a
                    ...
                    >
                      ...
                      <a
                        class="cursor-pointer"
                        v-on:click="fetchSomething((ordering = 'lineage'))"
                      >
                        &#x2193;
                      </a>
                      <a
                        class="cursor-pointer"
                        v-on:click="fetchSomething((ordering = '-ordering'))"
                      >
                        &#x2191;
                      </a>
                    </th>
                    ...
                    >
                      ...
                      <a
                        class="cursor-pointer"
                        v-on:click="fetchSomething((ordering = 'gene'))"
                      >
                        &#x2193;
                      </a>
                      <a
                    ...
                    >
                      ...
                      <a
                        class="cursor-pointer"
                        v-on:click="fetchSomething((ordering = 'reference_id'))"
                      >
                        &#x2193;
                      </a>
                      <a
                        class="cursor-pointer"
                        v-on:click="
                          fetchSomething((ordering = '-reference_id'))
                        "
                      >
                        &#x2191;
                      </a>
                    ...
                    >
                      ...
                      <a
                        class="cursor-pointer"
                        v-on:click="
                          fetchSomething((ordering = 'amino_acid_position'))
                        "
                      >
                        &#x2193;
                      </a>
                    ...
                    >
                      ...
                      <a
                        class="cursor-pointer"
                        v-on:click="fetchSomething((ordering = 'mutation'))"
                      >
                        &#x2193;
                      </a>
                      <a
                ...
                <tbody class="bg-white divide-y text-middle divide-gray-200">
                  <tr
                    v-for="event in ip.data.results"
                    :key="event.id"
                    :event="event"
                  >
                ...
                >
                  ...
                  <div>
                    ...
                    >
                      <a
                        ...
                        v-on:click="fetchSomething((page = page - 1))"
                      >
                        Previous
                      </a>
                      ...
                      <a
                        ...
                        v-on:click="fetchSomething((page = 1))"
                      >
                        1
                      </a>
                  ...
                  <a class="bg-white border-gray-300 text-gray-500 hover:bg-gray-50 relative inline-flex items-center px-4 py-2 border text-sm font-medium" v-on:click="fetchSomething(page=page+4)">
                    ...
                    {{ page+1 }}
                      ...
                      <a
                        v-if="page >= 1 && page < ip.data.count / 20"
...
<script>
import CubeSpin from '/home/nsm-07/Desktop/Bhupati/insacog-voc-filter/components/Cube.vue'
...
export default {
  ...
  data() {
    return {
      ...
      barChartOptions: {
        ...
        title: {
          display: true,
          text: 'State wise strain distribution',
          fontSize: 18,
          fontColor: '#6b7280',
        },
        tooltips: {
  ...
  async mounted() {
    ...
    const mutationdistribution = await axios.get(
      `${process.env.baseUrl}/statesmutationdistribution/`,
    )
    mutationdistribution.data.forEach((d) => {
      const { state, strain__count } = d

      this.arrStrain.push(strain__count)
      this.arrStates.push(state)
    })
    this.random = 456789
    this.barChartData.labels = this.arrStates
    this.barChartData.datasets[0].data = this.arrStrain
  },
  ...
  async asyncData() {
    ...
    const lineagescount = await axios.get(
      `${process.env.baseUrl}/uniqelineagecount/`,
    )
    const lineagestraindistribution = await axios.get(
      `${process.env.baseUrl}/uniquelineagestrain/`,
    )
    const ip = await axios.get(`${process.env.baseUrl}/data/?page=${page}`)
    const sequences = await axios.get(`${process.env.baseUrl}/count/`)
    const Genome_sequenced = await axios.get(
      `${process.env.baseUrl}/genomesseqenced/`,
    )
  ...
  methods: {
    ...
    async fetchSomething() {
      ...
      this.arrStrain.splice(0)

      if (page > 1) {
        const prev = page - 1
      }
      const ip = await axios.get(
        `${process.env.baseUrl}/data/?page=${this.page}&search=${this.search}&start_date=${this.start_date}&end_date=${this.end_date}&days=${this.days}&strain=${this.strain}&ordering=${this.ordering}&state=${this.state}&lineage=${this.lineage}&mutation_deletion=${this.mutation_deletion}&date=${this.date}&gene=${this.gene}&reference_id=${this.reference_id}&amino_acid_position=${this.amino_acid_position}&mutation=${this.mutation}`,
      )
      this.ip = ip

      const lineagescount = await axios.get(
        `${process.env.baseUrl}/uniqelineagecount/?search=${this.search}&days=${this.days}&start_date=${this.start_date}&end_date=${this.end_date}&strain=${this.strain}&state=${this.state}&lineage=${this.lineage}&mutation_deletion=${this.mutation_deletion}&date=${this.date}&gene=${this.gene}&reference_id=${this.reference_id}&amino_acid_position=${this.amino_acid_position}&mutation=${this.mutation}`,
      )
      this.lineagescount = lineagescount
      const lineagestraindistribution = await axios.get(
        `${process.env.baseUrl}/uniquelineagestrain/?search=${this.search}&days=${this.days}&start_date=${this.start_date}&end_date=${this.end_date}&strain=${this.strain}&state=${this.state}&lineage=${this.lineage}&mutation_deletion=${this.mutation_deletion}&date=${this.date}&gene=${this.gene}&reference_id=${this.reference_id}&amino_acid_position=${this.amino_acid_position}&mutation=${this.mutation}`,
      )
      this.lineagestraindistribution = lineagestraindistribution

      const Genome_sequenced = await axios.get(
        `${process.env.baseUrl}/genomesseqenced/?search=${this.search}&days=${this.days}&start_date=${this.start_date}&end_date=${this.end_date}&strain=${this.strain}&state=${this.state}&lineage=${this.lineage}&mutation_deletion=${this.mutation_deletion}&date=${this.date}&gene=${this.gene}&reference_id=${this.reference_id}&amino_acid_position=${this.amino_acid_position}&mutation=${this.mutation}`,
      )
      this.Genome_sequenced = Genome_sequenced
      const mutationdistribution = await axios.get(
        `${process.env.baseUrl}/statesequencesdistribution/?search=${this.search}&days=${this.days}&start_date=${this.start_date}&end_date=${this.end_date}&strain=${this.strain}&ordering=${this.ordering}&state=${this.state}&lineage=${this.lineage}&mutation_deletion=${this.mutation_deletion}&date=${this.date}&gene=${this.gene}&reference_id=${this.reference_id}&amino_acid_position=${this.amino_acid_position}&mutation=${this.mutation}`,
      )
      mutationdistribution.data.forEach((d) => {
        const { state, strain__count } = d
      ...
      const data1 = await axios.get(
        `${process.env.baseUrl}/monthlydistribution/?year=${this.year}&days=${this.days}&search=${this.search}&start_date=${this.start_date}&end_date=${this.end_date}&strain=${this.strain}&state=${this.state}&lineage=${this.lineage}&mutation_deletion=${this.mutation_deletion}&date=${this.date}&gene=${this.gene}&reference_id=${this.reference_id}&amino_acid_position=${this.amino_acid_position}&mutation=${this.mutation}`,
      )
      console.log(data1.data)
      // spinner.hide()
      data1.data.forEach((d) => {
        const { month_number, strain__count } = d
      ...
      const { data } = await axios.get(
        `${process.env.baseUrl}/distribution/?year=${this.year}&search=${this.search}&days=${this.days}&start_date=${this.start_date}&end_date=${this.end_date}&strain=${this.strain}&state=${this.state}&lineage=${this.lineage}&mutation_deletion=${this.mutation_deletion}&date=${this.date}&gene=${this.gene}&reference_id=${this.reference_id}&amino_acid_position=${this.amino_acid_position}&mutation=${this.mutation}`,
      )
      data.forEach((d) => {
        const { week_number, strain__count } = d
        this.arrweekdata.push(strain__count)
    ...
    async downloadFile() {
      const csv = await axios.get(
        `${process.env.baseUrl}/exportcsv/?days=${this.days}&search=${this.search}&start_date=${this.start_date}&end_date=${this.end_date}&strain=${this.strain}&ordering=${this.ordering}&state=${this.state}&lineage=${this.lineage}&mutation_deletion=${this.mutation_deletion}&date=${this.date}&gene=${this.gene}&reference_id=${this.reference_id}&amino_acid_position=${this.amino_acid_position}&mutation=${this.mutation}`,
      )
